...
class DetailDiscoverTests: XCTestCase {
    ...
    override func setUp() {
        discoverDetailService = DiscoverDetailService(serachCoreData: searchWordDoreDataManagerType, petCoreData: petKeywordCoreDataManagerType, network: networkManagerType, algorithm: algorithmManagerType)
    }
    ...
    func testGetAPI() {
        discoverDetailService?.getShoppingData(start: 1, search: "강아지간식", completion: { (isSuccess, _, _) in
            XCTAssert(isSuccess, "상품가져오기 실패")
        })
    }
