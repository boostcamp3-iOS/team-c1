...
class SearchServiceTests: XCTestCase {
    ...
    func testFetchRecommandSearchWord() {
        searchService.fetchRecommandSearchWord(completion: { (error) in
            if let error = error {
                XCTAssertNil(error, "Fetch Fail Error!! \(error)")
            } else {
                XCTAssert(self.searchService.keyword.count != 0)
            }
        })
    }

    func testSortChanged() {
        searchService.sortChanged(sort: sortOpt)
        sleep(1)
        XCTAssert(searchService.sortOption == sortOpt, "sortOpt 실패")
        XCTAssert(searchService.itemStart == 1, "itemStart 초기화 실패")
        XCTAssert(searchService.dataLists.count == 20, "아이템 인서트 실패")
    }

    func testSearchButtonClicked() {
        let search = MockShoppingNetworkManagerDummy.search
        searchService.searchButtonClicked(search)
        sleep(1)
        XCTAssert(searchService.sortOption == .similar, "sortOpt 초기화 실패")
        XCTAssert(searchService.itemStart == 1, "itemStart 초기화 실패")
        XCTAssert(searchService.dataLists.count == 20, "아이템 인서트 실패")
    }
    ...
    func testPagination() {
        ...
        sleep(1)
        XCTAssert(searchService.isInserting == true, "isInserting 바뀜 실패")
        XCTAssert(searchService.itemStart == 21, "itemStart 더하기 실패")
        XCTAssert(searchService.dataLists.count == 20, "아이템 인서트 실패")
    }

    func testCancelButtonClicked() {
        searchService.cancelButtonClicked()
        XCTAssert(searchService.dataLists.count == 0, "아이템 삭제 실패")
        XCTAssert(searchService.isInserting == false, "isInserting 초기화 실패")
    }

    func testSearchButtonClickedErrorEmpty() {
        searchService.searchButtonClicked(MockShoppingNetworkManagerDummy.failSearch)
        sleep(1)
        XCTAssert(searchService.sortOption == .similar, "sortOpt 초기화 실패")
        XCTAssert(searchService.itemStart == 1, "itemStart 초기화 실패")
        XCTAssert(searchService.dataLists.count == 0, "아이템 인서트 실패")
    }
...
class MockShoppingNetworkManager: NetworkManagerType {
    ...
    func getAPIData(_ params: ShoppingParams, completion: @escaping (APIResponseShoppingData) -> Void, errorHandler: @escaping (Error) -> Void) {
        var data = Data(MockShoppingNetworkManagerDummy.successDummyString.utf8)
        if params.search == "강아지 실패" {
            data = Data(MockShoppingNetworkManagerDummy.failDummyString.utf8)
        }
